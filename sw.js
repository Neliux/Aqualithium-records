const CACHE='swim-quest-final-20260916-7';
...
function patchGame(text){
  ...
  if(!out.includes('src="./final-fixes.js')){
    out=out.replace('</body>','<script src="./workout-clan-fix.js?v=20260916-7"></script><script src="./final-fixes.js?v=20260916-7"></script></body>');
  }
  return out;
}
...
if(url.pathname.endsWith('/index.html')||url.pathname.endsWith('/')){try{const r=await fetch(e.request,{cache:'no-store'});const text=await r.text();const patched=text.replace('</body>','<script src="./workout-clan-fix.js?v=20260916-7"></script></body>');const response=new Response(patched,{status:r.status,statusText:r.statusText,headers:{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-store'}});const c=await caches.open(CACHE);await c.put(e.request,response.clone());return response}catch(err){const cached=await caches.match(e.request);return cached||new Response('Swim Quest no disponible',{status:503})}}
